...
/**
 * Create the actors for the picture (Harold + hammer)
 */
void Picture::CreateActors()
{
    // Create Harold actor
    auto harold = std::make_shared<Actor>(L"Harold");

    // Create Harold's main body image drawable. Replace filename with your Harold image.
    auto haroldBody = std::make_shared<ImageDrawable>(L"HaroldBody", L"images/harold.png");

    // Set the root so transforms work properly
    harold->SetRoot(haroldBody);

    // Add the body to Harold's drawables
    harold->AddDrawable(haroldBody);

    // Set Harold's initial position in the scene (tweak these values)
    harold->SetPosition(wxPoint(200, 300));

    // --- Create the hammer drawable and attach it to Harold ---

    // Create hammer image drawable (ensure file exists at images/hammer.png)
    auto hammer = std::make_shared<ImageDrawable>(L"Hammer", L"images/hammer.png");
    hammer->SetPosition(wxPoint(20, 10));
    harold->AddDrawable(hammer);
